#!/usr/bin/env python3
"""BASIL counterpart of the paper's Figure 3 — SHAP magnitude by prediction type.

Mirrors the layout/styling of the original BABE Figure 3 (generated by
generate_figure1.ipynb cell 12) so the two figures can be placed side-by-side
in the paper.

Usage:
  cd "SHAP Analysis Bias Models"
  .venv/bin/python basil_fig3_counterpart.py

Reads:  outputs/basil_shap/basil_shap_magnitude.csv
Writes: outputs/basil_shap/basil_fig3_counterpart.png  (300 DPI)
"""
...
OUT_PATH = ROOT / "outputs" / "basil_shap" / "basil_fig3_counterpart.png"
OUT_PDF  = ROOT / "outputs" / "paperReady" / "basil_fig3_counterpart.pdf"

# Match original BABE Fig 3 palette (generate_figure1.ipynb cell 7).
# FN added as a fourth category — orange chosen to be visually distinct
# from TP-green, FP-red, TN-blue while reading as a "warning"/error class.
...
# Match the y-axis range of the original BABE Fig 3 so the two figures
# are visually directly comparable. Outliers above this are clipped.
Y_MAX = 0.15
...
def main():
    ...
    for suffix, display in MODEL_ORDER:
        for pred_type in TYPE_ORDER:
            data = df[(df.model == suffix) & (df.category == pred_type)]["mean_abs_shap"].values
    ...
    ax.set_title("SHAP Magnitude Distributions by Prediction Type (BASIL)",
    ...
    # Quick print of n + median + mean per box so the figure is reproducible.
